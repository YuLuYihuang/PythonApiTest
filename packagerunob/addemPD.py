# -*-coding:utf-8 -*-
import requests

def postaddem(em):
    url = "https://stargate.ele.me/delimont.saas_tnt/employee/addInitEmployee"
    headers = {
        'Cookie': "PASSPORT_DELIMONT_TOKEN=PBE_2.0_",
        'Content-Type': "application/json",
        'X-shard': "shardid=2",
        'cache-control': "no-cache",
        'Postman-Token': "6d65ff2d-f07d-404d-84be-29e741885844,f7d0334c-7666-4951-a86d-c9cadbff2c29"
    }
    print '当前em',em.decode('string_escape')
    response = requests.request("POST", url, data=em, headers=headers)
    print response.text


payload = [
        "{\"employee\": {\"tenantId\": 33,\"name\": \"黄三金\",\"account\": \"yijin.huag@ele.me\",\"gender\": -1,\"email\": \"yijin.huang@ele.me\",\"phone\": \"12819846712\"}}\n",
        "{\"employee\": {\"tenantId\": 33,\"name\": \"黄四金\",\"account\": \"jin.hang@ele.me\",\"gender\": -1,\"email\": \"jin.huang@ele.me\",\"phone\": \"13618846522\"}}\n"
    ]

for d in payload:
    print '当前payload:', d.decode('string_escape')
    postaddem(d)
